define -> -> 
	...
	class Frame
		...
		@close: (frameEl, animate=true) ->
			...
			if animate
				frameEl.fadeOut (if _.isNumber(animate) then animate else 200), ->
		...
		showFixedAbove: (el) ->
			...
			@update = => Frame.fixFrameAboveAndCentered el, @el
			@update()
			Frame.show @el

		showPositionedAbove: (el) ->
			@el.appendTo document.body unless @shown()
			Frame.positionFrameAboveAndCentered el, @el
			Frame.show @el

		showAbove: (el) ->
			@el.appendTo document.body unless @shown()
			@update = => Frame.setFrameAboveAndCentered el, @el
			@update()
			Frame.show @el
