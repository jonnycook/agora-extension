define -> d: ['SiteInjector', 'views/ShoppingBarView'], c: ->
	class StaplesSiteInjector extends SiteInjector
		siteName: 'Staples'
		run: ->
			@initPage =>
				...
				$ =>
					if $('#largeProductImage').length
						currentSid = -> /^http:\/\/www\.staples\.com\/.*?\/product_([^\?]+)/.exec(document.location.href)[1]
						@waitFor '#largeProductImage', => #img tag that is child of #viewL element
							@initProductEl $('#largeProductImage'), productSid:currentSid()