...
define(['View', 'Site', 'Formatter', 'util'], function(View, Site, Formatter, util) {
  ...
  return ProductPriceView = (function(superClass) {
    extend(ProductPriceView, superClass);
      ...
      return this.resolveObject(args, (function(_this) {
        return function(product) {
          ...
          if (indexOf.call(site.features, 'offers') >= 0) {
            ...
            update = function() {
              if (_this.product.get('offer')) {
                currentPrice = _this.product.get('offer').price;
                offerData.current.set({
                  price: _this.product.get('displayUserPrice'),
                  siteName: _this.product.get('offer').site,
                  siteIcon: _this.product.get('offer').url.match('(^https?://[^/]*)')[0] + '/favicon.ico',
                  url: util.url(_this.product.get('offer').url),
                  clear: true
                });
              } else {
                currentPrice = _this.product.get('price');
                offerData.current.set({
                  price: _this.product.get('displayPrice'),
                  siteName: site.name,
                  siteIcon: site.icon,
                  url: _this.product.get('url')
                });
              }
              return _this.product.field('offers')["with"](function(offers) {
                var icon, match;
          ...
          update();
          if (indexOf.call(site.features, 'offers') >= 0) {
            _this.ctx.observe(_this.product.field('offer'), update);
          }
          _this.ctx.observe(_this.product.field('price'), update);
    ...
    ProductPriceView.client = function() {
      return ProductPriceView = (function(superClass1) {
        extend(ProductPriceView, superClass1);
...
//# sourceMappingURL=ProductPriceView.js.map
