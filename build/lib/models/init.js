...
define(['models/Product', 'models/ProductVariant', 'models/Composite', 'model/Table', 'model/Database', 'model/ModelManager', 'Site', 'model/ObservableArray', 'model/auxiliary/maintainOrder', 'model/HasManyRelationship', 'util'], function(Product, ProductVariant, Composite, Table, Database, ModelManager, Site, ObservableArray, maintainOrder, HasManyRelationship, util) {
  return function(background) {
    ...
    addElementType = function(args) {
      var fields, graph, lc, model, parentKey, parentTable, ref, referents, relationships, table, types;
      ...
      graph = {
        element_id: {
          ...
          owns: function(record) {
            var ref;
            return !((ref = record.table.name) === 'products' || ref === 'product_variants');
          }
        }
      };
      if (args.graph) {
      ...
      if (args.hasParent) {
        relationships.parent = {
          type: 'hasOne',
          relKey: parentKey,
          model: (ref = args.parentModel) != null ? ref : args["for"]
        };
      }
      ...
      Composite: {
        ...
        relationships: {
          ...
          contents: function(instance) {
            var j, len, list, onInstance, ref, rel;
            ...
            for (j = 0, len = ref.length; j < len; j++) {
              ...
              instance.get(rel).observe(function(mutationInfo) {
                ...
                if (mutationInfo.type === 'insertion') {
                  ...
                  if (list.length() > 1) {
                    results = [];
                    for (i = k = ref1 = Math.min(instance.get('index'), list.length() - 1), ref2 = list.length(); ref1 <= ref2 ? k < ref2 : k > ref2; i = ref1 <= ref2 ? ++k : --k) {
                      results.push(list.get(i).set('index', i));
                    }
                    return results;
                  }
                }
    ...
    isObjectInShoppingBar = function(obj) {
      var beltElement, beltElements, bundleElement, bundleElements, decision, j, k, l, len, len1, len2, listElement, listElements, response;
      if (beltElements = modelManager.getModel('BeltElement').findAll({
        ...
        if (beltElements.length) {
          response = {};
          for (j = 0, len = beltElements.length; j < len; j++) {
            beltElement = beltElements[j];
      ...
      for (k = 0, len1 = listElements.length; k < len1; k++) {
        listElement = listElements[k];
      ...
      for (l = 0, len2 = bundleElements.length; l < len2; l++) {
        bundleElement = bundleElements[l];
    ...
    isProductInShoppingBar = function(product) {
      var beltElement, beltElements, bundle, decision, i, j, k, l, len, list, m, ref, ref1, ref2, response;
      if (beltElements = modelManager.getModel('BeltElement').findAll({
      ...
      for (i = k = 0, ref = product.get('lists').length(); 0 <= ref ? k < ref : k > ref; i = 0 <= ref ? ++k : --k) {
      ...
      for (i = l = 0, ref1 = product.get('bundles').length(); 0 <= ref1 ? l < ref1 : l > ref1; i = 0 <= ref1 ? ++l : --l) {
        bundle = product.get('bundles').get(i);
        if (response = isObjectInShoppingBar(bundle)) {
          return response;
        }
      }
      for (i = m = 0, ref2 = product.get('variants').length(); 0 <= ref2 ? m < ref2 : m > ref2; i = 0 <= ref2 ? ++m : --m) {
