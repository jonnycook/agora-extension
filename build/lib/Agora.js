...
define(['underscore', 'Debug', 'client/ContentScript', 'client/SiteInjector', 'Site', 'siteConfig', 'View', 'clientInterface/ClientValue', 'models/init', 'model/Event', 'Updater2', 'Public', 'views/ProductPriceView', 'views/ProductMenuView', 'views/SettingsView', 'CodeManager', 'Chat'], function(_, Debug, ContentScript, SiteInjector, Site, siteConfig, View, ClientValue, initModels, Event, Updater, Public, ProductPriceView, ProductMenuView, SettingsView, CodeManager, Chat) {
  ...
  return Agora = (function() {
    function Agora(background, opts) {
      var autoUpdate, createView, createViewQueue, db, lastUpdated, modelManager, onLoaded, scrapersTime, settingName, settings, tutorialCheck, updateScrapers, updateScrapersCbs, updateScrapersId, updater, updatingScrapers, user, _i, _len, _ref, _ref1;
      ...
      _ref = initModels(this.background), db = _ref.db, modelManager = _ref.modelManager;
      this.db = db;
      ...
      opts.localTest = (_ref1 = env.localTest) != null ? _ref1 : opts.localTest;
      this.View = View;
      View.agora = this;
      Site.agora = this;
          ...
          return function(cb) {
            ...
            return background.httpRequest('http://ext.agora.sh/getScrapers.php', {
              ...
              cb: function(response) {
                var i, newScraper, scraper, toRemove, _j, _k, _l, _len1, _len2, _len3, _m, _ref2, _ref3, _ref4;
                background.declarativeScrapers;
                toRemove = [];
                _ref2 = response.scrapers;
                for (_j = 0, _len1 = _ref2.length; _j < _len1; _j++) {
                  newScraper = _ref2[_j];
                  _ref3 = background.declarativeScrapers;
                ...
                if (toRemove.length) {
                  for (i = _l = _ref4 = toRemove.length - 1; _ref4 <= 0 ? _l <= 0 : _l >= 0; i = _ref4 <= 0 ? ++_l : --_l) {
                ...
                for (_m = 0, _len3 = updateScrapersCbs.length; _m < _len3; _m++) {
                  cb = updateScrapersCbs[_m];
                  cb(true);
                }
      ...
      onLoaded = (function(_this) {
        return function(success) {
          ...
          doOnLoaded = function() {
            var args, config, name, observeField, observeRecord, siteName, siteSettings, table, _j, _len1, _ref2;
            ...
            if (!env.core) {
              ...
              if (opts.client) {
                ...
                observeRecord = function(record) {
                  var field, _j, _len1, _ref2, _results;
                  if (record.table.schema.fields) {
                    _ref2 = record.table.schema.fields;
                    _results = [];
                    for (_j = 0, _len1 = _ref2.length; _j < _len1; _j++) {
                      field = _ref2[_j];
                ...
                _this.background.getStorage(['options'], function(data) {
                  ...
                  if (data.options) {
                    _results = [];
                    for (_j = 0, _len1 = settings.length; _j < _len1; _j++) {
                      setting = settings[_j];
                      _results.push(_this.settings[setting].set(data.options[setting]));
                    }
                    return _results;
                  }
                });
            ...
            for (_j = 0, _len1 = createViewQueue.length; _j < _len1; _j++) {
              args = createViewQueue[_j];
        ...
        tutorialCheck = (function(_this) {
          return function(tutorial) {
            if (_this.user.get('tutorials')) {
              if (__indexOf.call(_this.user.get('tutorials').split(' '), tutorial) >= 0) {
                return false;
              } else {
                return true;
              }
          ...
          return function(source, tutorial, sendResponse) {
            ...
            if (env.core) {
              ...
              if (_.isArray(tutorial)) {
                for (_j = 0, _len1 = tutorial.length; _j < _len1; _j++) {
                  t = tutorial[_j];
                  if (tutorialCheck(t)) {
                    sendResponse(t);
                    return;
    ...
    Agora.prototype._load = function(classes, cb, classDefs) {
      ...
      if (deps.length) {
        return this.background.require(_.map(deps, function(className) {
          ...
          return function() {
            var c, i, index, _i, _ref;
            c = {};
            for (i = _i = 0, _ref = deps.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
              className = deps[i];
    ...
    Agora.prototype._compileContentScript = function(classDefs, site) {
      var body, classCode, classVars, name, setting, settingName, settingsStr, _i, _len, _ref, _ref1;
      classVars = classDefs.join('\n');
      classCode = "var __classes = {};\n";
      for (_i = 0, _len = classDefs.length; _i < _len; _i++) {
        _ref = classDefs[_i], name = _ref.name, body = _ref.body;
        classCode += "window." + name + " = __classes." + name + " = (" + body + ")();\n";
      }
      settingsStr = '';
      _ref1 = this.settings;
      for (settingName in _ref1) {
        setting = _ref1[settingName];
    ...
    Agora.prototype._setSiteEnabled = function(siteID, enabled) {
      ...
      if ((_base = this.siteSettings.get())[siteID] == null) {
        _base[siteID] = {};
      }
    ...
    Agora.prototype.setOptions = function(options) {
      return this.background.getStorage(['options'], (function(_this) {
        return function(data) {
          var prevOptions, prop, value, _ref;
          prevOptions = (_ref = data.options) != null ? _ref : {};
          for (prop in options) {
            value = options[prop];
    ...
    Agora.prototype.signalReload = function() {
      ...
      if (this.tabs) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          tab = _ref[_i];
          _results.push(chrome.tabs.sendMessage(tab, 'needsReload'));
        }
        return _results;
      }
    };
    ...
    Agora.prototype.onInit = function(success) {
      ...
      if (this.updater.userId) {
        if (success) {
          ...
          if ((_ref = this.View.views.ShoppingBar) != null ? _ref["null"] : void 0) {
