...
define(['jQuery', 'Background', 'ChromeContentScript'], function($, Background, ChromeContentScript) {
  ...
  return ChromeBackground = (function(superClass) {
    extend(ChromeBackground, superClass);
        ...
        return function(port) {
          if (port.sender.tab) {
            ...
            return port.onMessage.addListener(function(message) {
              var j, len, ref, request, results;
              if (message.requests) {
                ref = message.requests;
                results = [];
    ...
    ChromeBackground.prototype.injectUtilScripts = function(sender, done) {
      var _loadScripts, cb, count, loadScripts;
      _loadScripts = function(scripts, cb) {
        var count, j, len, results, script, tick;
        ...
        for (j = 0, len = scripts.length; j < len; j++) {
          script = scripts[j];
          results.push(chrome.tabs.executeScript(sender.id, {
            file: script
          }, tick));
        }
        return results;
...
//# sourceMappingURL=ChromeBackground.js.map
